...
| T-01-03-acct | Tampering / Elevation of Privilege | GitHub/Vercel/registrar accounts | high | transfer | Cannot be mitigated in code. Documented as named, unchecked manual rows in root `SECURITY.md` Part 3 with exact settings path and responsible owner (D-06). Repository visibility resolved `public` by user decision at the Task 2 checkpoint — SEC-12/13/14 are GitHub-native-satisfiable once the repo is created; remote creation and push are explicitly deferred pending the user's file-list audit approval | closed (transfer) |
